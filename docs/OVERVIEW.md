...
- **«John Doe, что ты думаешь?»** — Saymo слышит твоё имя в звонке и отвечает за тебя из подготовленного контекста
...
3. `addressing` проверяет, что фраза действительно адресована тебе, а не просто упоминает имя
4. Если это типовой вопрос — берёт готовый ответ из Tier-A response cache
5. Если cache miss и `responses.live_fallback: true` — отдаёт вопрос в Ollama и TTS
6. BlackHole 2ch отправляет ответ в звонок

Имена и варианты — в `~/.saymo/config.yaml`. Никакого хардкода в исходниках.
Проверить настройку без звонка можно через `saymo trigger-check -p standup --text "John, что по статусу?"`.
